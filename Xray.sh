...
#	Description: Install the Xray server
#	Version: 1.0.0.1
#	Author: 77-QiQi
...
Installation_dependency(){
	...
	mkdir -p $config_folder/Xray
	mkdir -p $config_folder/nginx/conf.d
	mkdir -p $config_folder/nginx/html
	curl https://raw.githubusercontent.com/77-QiQi/docker-Xray/main/data/conf/config.json -o $config_folder/conf/config.json
	curl https://raw.githubusercontent.com/77-QiQi/docker-Xray/main/data/conf/docker-compose.yml -o $config_folder/conf/docker-compose.yml
	curl https://raw.githubusercontent.com/77-QiQi/docker-Xray/main/data/conf/info.conf -o $config_folder/conf/info.conf
	curl https://raw.githubusercontent.com/77-QiQi/docker-Xray/main/data/conf/nginx.conf -o $config_folder/conf/nginx.conf
	curl https://raw.githubusercontent.com/77-QiQi/docker-Xray/main/data/conf/index.html -o $config_folder/conf/index.html
...
Install_Xray(){
	check_root
	[[ -e ${config_file} ]] && echo -e "${Error} Xray 配置文件已存在，请检查( 如安装失败或者存在旧版本，请先初始化/卸载 ) !" && return 1
	if ! [ -x "$(command -v docker-compose)" ]; then
	  echo -e "${Error}: docker-compose is not installed." >&2
	  return 1
	fi
	echo -e "${Info} 开始设置 Xray 配置..."
	...
	[[ -z ${ports} ]] && ports="443"
	if [[ $ports == "22" || $ports == "25" || $ports == "80" || $ports == "3306" ]]; then
	    echo
	    echo -e "${Error} 端口不能为 22,25,80,3306 ..." && return 0
	fi
	read -r -p "请输入路径(eg:xray):" paths
	if [ -z $paths ];then
	...
	rm -f $config_folder/Xray/config.json && cp $config_folder/conf/config.json $config_folder/Xray/config.json
	...
	sed -i "6s/xray/${paths}/" $folder/info.conf
	...
	sed -i "s/your_uuid/${uuid}/" $config_folder/Xray/config.json
	sed -i "s/your_domain/${domains}/" $config_folder/Xray/config.json
	sed -i "61s/xray/${paths}/" $config_folder/Xray/config.json
	sed -i "34s/xray/${paths}/" $config_folder/nginx/conf.d/nginx.conf
	...
	echo "Xray 配置信息"  >> $folder/check_info.conf
	echo "地址（address）：${domains}"  >> $folder/check_info.conf
	echo "端口（port）：${ports} / 3306"  >> $folder/check_info.conf
	...
	echo "传输协议（network）：ws / tcp"  >> $folder/check_info.conf
...
Install_tls(){
	...
	echo "   server_tokens off;"  >> $config_folder/nginx/conf.d/nginx.conf
	...
	docker-compose up --force-recreate -d nginx xray
...
Update_Service(){
	[[ ! -e ${config_file} ]] && echo -e "${Error} 没有安装 Xray ，请检查 !" && return 1
	if ! [ -x "$(command -v docker-compose)" ]; then
	  echo -e "${Error}: docker-compose is not installed." >&2
	  return 1
	fi
...
Domains_Setting(){
	...
	sed -i "s/${domains}/${new_domains}/" $config_folder/Xray/config.json
...
Ports_Setting(){
	read -r -p "请输入端口号(当前默认端口:${ports}):" new_ports
		...
		if [[ $new_ports == "22" || $new_ports == "25" || $new_ports == "80" || $new_ports == "3306" ]]; then
		echo
		echo -e "${Error} 端口不能为 22,25,80,3306 ..." && return 0
		fi
...
Paths_Setting(){
	...
	sed -i "61s/${paths}/${new_paths}/" $config_folder/Xray/config.json
...
uuid_Setting(){
	...
	sed -i "s/${uuid}/${new_uuid}/" $config_folder/Xray/config.json
	cd $folder
	docker-compose up --force-recreate -d nginx xray
	cd ~
	return 0
...
Uninstall_Xray(){
	[[ ! -e ${config_folder} ]] && [[ ! -e ${config_file} ]] && echo -e "${Error} 没有安装 Xray ，请检查 !" && return 1
	echo "确定要 卸载 Xray ？[y/N]" && echo
...
Restart_Xray(){
	...
	echo -e "${Info} 重启容器..."
	docker restart nginx xray
	echo -e "${Info} done..."
	return 0
}
View_connection_info(){
	[[ ! -e ${config_file} ]] && echo -e "${Error} Xray 配置文件不存在，请检查 !" && return 1
...
Update_Shell(){
	curl https://raw.githubusercontent.com/77-QiQi/docker-Xray/main/Xray.sh -o Xray.sh && echo -e "${Info} 更新完成 ..."
	source Xray.sh
}
...
check_root
echo -e "  Xray 一键管理脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
  ---- 77-QiQi | github.com/77-QiQi ----

  ${Green_font_prefix}0.${Font_color_suffix} 安装 依赖
  ${Green_font_prefix}1.${Font_color_suffix} 安装 Xray
  ${Green_font_prefix}2.${Font_color_suffix} 更新 Xray
  ${Green_font_prefix}3.${Font_color_suffix} 卸载 Xray
————————————
...
————————————
  ${Green_font_prefix}7.${Font_color_suffix} 停止 Xray
  ${Green_font_prefix}8.${Font_color_suffix} 重启 Xray
————————————
...
echo -e "${Red_font_prefix} [注意：初始化，将恢复至 Xray 安装之前(依赖安装之后)的状态！] ${Font_color_suffix}"
